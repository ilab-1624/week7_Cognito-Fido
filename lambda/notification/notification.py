...
class AlertNotify():
    def __init__(self, personImageMessages, validationResultMessage):
        ...
        self.__cognito_client = boto3.client('cognito-idp',
            region_name = config.REGION_NAME, 
            aws_access_key_id = config.AWS_ACCESS_KEY_ID,
            aws_secret_access_key = config.SECRET_ACCESS_KEY
            )
    ...
    def getUserLineID(self):
            #listIDlist= []
            personDatalist = self.__validationResultMessage.getPersonList()
            for person in personDatalist:
                print(person)
                if person["isMember"]:
                    try:
                        searchResponse = self.__cognito_client.list_users(
                            UserPoolId=config.USER_POOL_ID,
                            AttributesToGet=config.attributeList,
                            Filter="username=\"" + person['memberId'] + "\""
                        )
                        print(searchResponse)
                        self.__receiverLineIdList.append(searchResponse['Users'][0]['Attributes'][0]['Value'])
                    except:
                        print("query_error")
            '''
            userData = {
                'username': searchResponse['Users'][0]['Attributes'][1]['Value'],
                'attributes': searchResponse['Users'][0]['Attributes']
            }
                '''
        #print(searchResponse)
        #self.__receiverLineIdList = listIDlist
    ...
    def pushMessages(self):
        ...
        try:
            for receiverLineId in self.__receiverLineIdList:
                print(receiverLineId)
                lineBotApi.push_message(receiverLineId, pushMessages)
                pushResult = 'Success'
